...
\begin{figure}
 \centering
 \subfloat[\centering Horizontale Skalierung]{{\includegraphics[width=.4\textwidth]{\figdir/horizontal-scaling} }}%
 \qquad
 \subfloat[\centering Vertikale Skalierung]{{\includegraphics[width=.5\textwidth]{\figdir/vertical-scaling} }}%
 \caption{Skalierung \cite{Slingerland.2023}}%
 \label{FIG:scaling}%
\end{figure}
Durch vertikale Skalierung wird zum Beispiel die Anzahl der virtuellen Maschinen in einem Cluster erhöht oder verringert, um das System auf eine steigende oder sinkende Nachfrage anzupassen.
...
Bei der horizontalen Skalierung wird beispielsweise bei einer vorhandenen virtuellen Maschine die CPU oder die Speicherkapazität erhöht oder verringert.
...
Auto-Scaling wird detaillierter in Kapitel~\ref{CAP:strategies} behandelt, wenn es um die Anwendung der hier vorgestellten Strategien geht.
% Load shifting
...
\begin{itemize}
 \item Repräsentativer Charakter: In die Entscheidung fließt die Verfügbarkeit von Rechenzentren der drei größten öffentlichen Cloud-Anbieter - \ac{AWS}, Microsoft Azure und \ac{GCP} - in diesen Ländern ein.
...
\begin{figure}[ht]
 ...
 \caption{Die Durchschnittswerte der verwendeten Wetterdaten pro Monat (eigene Darstellung)}%
...
\begin{figure}
 \caption{Der Anteil der Energiequellen an der Stromproduktion in Deutschland und Norwegen im Jahr 2023 (eigene Darstellung, nach~\cite{ElectricityMaps.20240305T20:54:29.000Z})}
...
\begin{figure}
 \caption{Die Höhe der MOER des deutschen und norwegischen Stromnetzes pro Monat (eigene Darstellung)}
 {\includegraphics[width=0.99\textwidth]{\figdir/moer_trend_month}}
...
Die vorherige Dekomposition der Daten (Abbildung~\ref{FIG:} weist darauf hin, dass sie stationär sind, jedoch soll diese Vermutung nochmals bestätigt werden.
...
Die partielle Auto-Korrelation ist nach der ersten Zeitverzögerung nahe 1, was bedeutet, dass die Werte an einem Tag denen des vorherigen Tags ähnlich sind.
...
Die Frequenz wird in dieser Implementierung als Parameter \lstinline[columns=fixed]{m} angegeben, entspricht aber dem sonst verwendet \lstinline[columns=fixed]{s} und wird der täglichen Saisonalität wegen auf 24 gesetzt.
...
\begin{figure}
 \caption{Diagnostik des SARIMAX-Modells zur Prognose der MOER des deutschen Stromnetzes (eigene Darstellung)}
...
Wie in der Beispielimplementierung von \textit{pytorch forecasting} (\cite{GitHub.20240307T20:56:16.000Z}) werden für das Training alle Daten bis auf die letzten der
...
\begin{table}[t]
 ...
 \input{\tabledir/HyperparameterTrained.tex}
\end{table}
...
\begin{table}[t]
 ...
 \input{\tabledir/HyperparameterUsed.tex}
\end{table}

\section{Evaluierung und Vergleich der Modelle}
Die Modelle sollen anhand des \ac{MAE}, des \ac{MSE} und des \ac{MAPE} evaluiert und miteinander verglichen werden.
Die Vorhersageleistung der Modelle ist in Tabelle~\ref{TAB:forecasting-performance} zusammengefasst.
Die Werte sind auf zwei Nachkommastellen gerundet angegeben.
\begin{table}[ht]
 \centering
...
Zur Anwendung der im vorherigen Kapitel beschriebenen Prognose der Kohlenstoffintensität sollen bereits verfügbare Tools, die zur Ausrichtung von Software an Emissionswerte verwendet werden könnten, betrachtet werden.
Dafür werden die Möglichkeiten der drei gängigsten Public-Cloud-Plattformen - \ac{AWS}, Azure und \ac{GCP} - untersucht.
...
Da in diesem Zusammenhang wegen seiner hohen Individualisierbarkeit und Unabhängigkeit oft das Container-Orchestrierungstool Kubernetes Anwendung findet, wird dieses ebenso auf Möglichkeiten zur Ausrichtung von Software nach Emissionen analysiert.
...
Neben bestimmten vordefinierten Metriken, wie zum Beispiel CPU-Auslastung oder Anzahl der Threads können auch benutzerdefinierten Metriken, die direkt von der jeweiligen Anwendung generiert werden, verwendet werden.
Darüber hinaus können auch Zeitpläne zur Auslösung der Skalierung verwendet werden.
...
Kubernetes verfügt über den Horizontal Pod Autoscaler
...
AWS Lambda ist ein Serverless Computing-Service, also ein Service, der die Ausführung von Code ohne Serverbereitstellung oder -verwaltung ermöglicht.
...
Im Folgenden soll beispielhaft betrachtet werden, wie die in Kapitel~\ref{CAP:prediction} beschriebenen Vorhersagen Anwendung finden können.
Die Abfrage der Prognosen soll durch eine \ac{API} ermöglicht werden.
...
Dafür werden einige Annahmen getroffen:
\begin{itemize}
 \item Die ungefähre Dauer des Workloads ist bekannt. Sie wird in Minuten und speziell für die verwendete Umgebung angegeben.
\end{itemize}

Um die \ac{API} mit so wenig Overhead wie möglich umzusetzen, wird Flask, ein Mikro-Frontend zur einfachen Erstellung von Webanwendungen und \ac{API}s mit Python, verwendet~\cite{.20240203T21:13:11.000Z}.
Zur Konfiguration der Abfrage sollen folgende Parameter angegeben werden:
Welches Prognose-Modell verwendet werden soll (\ac{SARIMAX} oder \ac{TFT}, default ist \ac{TFT}), die geschätzte Bearbeitungszeit des Workloads in Minuten, ein Fälligkeitsdatum und das Land (Deutschland oder Norwegen).
...
Da es sich um ein Konzept handelt, bestehen folgende Einschränkungen:
\begin{itemize}
