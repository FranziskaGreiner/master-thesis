...
Dies sind von verschiedenen Quellen, wie Satelliten oder Wetterstationen, gemessene Daten, bei denen fehlende Werte durch physikalische Gesetze oder historische Verlaufsmuster generierte Daten ergänzt wurden.
...
Im betrachteten Anwendungsfall der Vorhersage von \ac{MOER}-Werten sind die bekannten Variablen die Wetterdaten, sowie die jeweilige Jahreszeit, der Wochentag und, ob es sich um einen Feier-/Ferientag handelt.
...
Verglichen werden soll das \ac{TFT}-Modell mit einem traditionellen, statistischen Modell, dem SARIMAX-Modell.
Das SARIMAX-Modell kommt folgendermaßen zustande:
...
Das ARIMA-Modell in Abhängigkeit von der Ordnung des AR-Prozesses $p$, der Ordnung der Integration $d$ und der Ordnung des MV-Prozesses $q$, kann durch Formel~\ref{eq:arima} ausgedrückt werden~\cite{Peixeiro.2022}.
...
\begin{equation}
 \label{eq:sarimax}
 d_t = C + \sum_{n=1}^{P} \phi_n d_{t-sn} + \sum_{n=1}^{q} \theta_n e_{t-n} + \varepsilon_t + \sum_{n=1}^{p} \alpha_n d_{t-n} + \sum_{n=1}^{Q} \eta_n e_{t-sn} + \sum_{n=1}^{r} \beta_n x_{t-n}
\end{equation}
Ergänzt wurde die Formel durch die Saisonalität in Form einer zusätzlichen AR- ($$) und MA- ($$) Komponente verschoben um die Frequenz der Saisonalität (z.B.\ 12: monatlich oder 24: stündlich) und um die exogenen Variablen ($\sum_{n=1}^{r} \beta_n x_{t-n}$)~\cite{Artley.26.4.2022}.
