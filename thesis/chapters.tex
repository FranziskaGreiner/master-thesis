...
Eine einfache Anwendung gemachter Vorhersagen ist von großem Vorteil.
Die Green Software Foundation hat ein \ac{SDK} entwickelt, mit Hilfe dessen herausgefunden werden kann, wann und wo ein Workload mit der geringsten Menge an Kohlenstoff ausgeführt werden kann.
...
Täglich vergleicht Google die Vorhersage der erwarteten durchschnittlichen Kohlenstoffintensität im Verlauf des nächsten Tages (verwendet wir die Vorhersage von Electricity Maps) mit der Vorhersage der auf dem System zu erwartenden Last und passt diese beiden Größen aufeinander an.
...
Rundum verfügbare Services, wie die Google-Suche oder der Kartendienst Google-Maps sollen davon nicht beeinträchtigt werden, doch für nicht zeitkritische Updates, wie das Hinzufügen neuer Wörter zum Google-Übersetzer können verschoben werden.
...
Die Schwierigkeit bei der Datenerfassung besteht darin, dass zwei Fälle miteinander verglichen werden müssen.
...
Eine Lösung dieses Problems ist, die Netzbedingungen mitzubetrachten und davon auszugehen, dass bei gleichen Bedingungen auch die MOER gleich ist.
...
In Abbildung~\ref{FIG:moer_distribution} wird ersichtlich, dass Norwegen eine weitaus geringere Kohlenstoffintensität hat als Deutschland.
\begin{figure}
...
Konkret wurden vier mögliche Einflussfaktoren für die Kohlenstoffintensität als wichtig identifiziert und ausgewählt:
...
%  \centering
%  \includegraphics[width=.4\linewidth]{\figdir/acf_moer_de}
% \caption{Die ACF der MOER-Werte für Deutschland}
% \label{FIG:acf_moer_de}
%\end{subfigure}
%\begin{subfigure}{.5\textwidth}
% \centering
% \includegraphics[width=.4\linewidth]{\figdir/pacf_moer_de}
% \caption{Die PACF der MOER-Werte für Deutschland}
% \label{FIG:pacf_moer_de}
%\end{subfigure}
%\end{figure}
...
Durch die Definition eines Zeitplans kann ein Job in festgelegten Intervallen ausgeführt werden~\cite{cronjob}.
...
Zudem können zum Beispiel bei der Auto-Scaling Funktion von Amazon EC2-Instanzen mehrere Regionen für die Verteilung der Ressourcen angegeben werden, um eine hohe Ausfallsicherheit zu garantieren~\cite{amazonwebservices}.
